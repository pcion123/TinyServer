...
import com.tinybee.platform.mapper.PlatformAccountMapper;
...
@Service
public class PlatformAccountServiceImpl implements AccountService
{
	private Logger mLogger = LoggerFactory.getLogger(PlatformAccountServiceImpl.class);
	...
	public Account getAccount(String acc) throws AccountException
	{
		...
		try
		{
			account = redisManager.get(Account.class, String.format("account:platform:%s", acc));
			if (account != null)
				return account;
		}
		catch (Exception e)
		...
		try
		{
			...
			if (account != null)
			{
				redisManager.setExpire(String.format("account:platform:%s", acc), account, 30 * 60);
			}
		}
		catch (Exception e)
		{
