...
import com.tinybee.platform.mapper.FacebookAccountMapper;
...
@Service
public class FacebookAccountServiceImpl implements AccountService
{
	private Logger logger = LoggerFactory.getLogger(FacebookAccountServiceImpl.class);
	...
	@Autowired
	private FacebookAccountMapper mapper;
	@Autowired
	...
	public Account getAccount(String acc) throws AccountException
	{
		...
		try
		{
			account = redisManager.get(Account.class, String.format("account:facebook:%s", acc));
			if (account != null)
				return account;
		}
		catch (Exception e)
		...
		try
		{
			...
			if (account != null)
			{
				redisManager.setExpire(String.format("account:facebook:%s", acc), account, 30 * 60);
			}
		}
		catch (Exception e)
		{
