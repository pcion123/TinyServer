...
import com.tinybee.platform.mapper.GoogleAccountMapper;
...
@Service
public class GoogleAccountServiceImpl implements AccountService
{
	private Logger logger = LoggerFactory.getLogger(GoogleAccountServiceImpl.class);
	...
	@Autowired
	private GoogleAccountMapper mapper;
	@Autowired
	...
	public Account getAccount(String acc) throws AccountException
	{
		...
		try
		{
			account = redisManager.get(Account.class, String.format("account:google:%s", acc));
			if (account != null)
				return account;
		}
		catch (Exception e)
		...
		try
		{
			...
			if (account != null)
			{
				redisManager.setExpire(String.format("account:google:%s", acc), account, 30 * 60);
			}
		}
		catch (Exception e)
		{
