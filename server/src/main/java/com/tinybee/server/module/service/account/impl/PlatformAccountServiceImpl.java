...
import com.tinybee.server.mapper.PlatformAccountMapper;
...
public class PlatformAccountServiceImpl implements IAccountService
{
	private static Logger mLogger = LoggerFactory.getLogger(PlatformAccountServiceImpl.class);
	
	private static IAccountService mInstance;
	
	private PlatformTransactionManager mTransactionManager;
	private PlatformAccountMapper mMapper;
	...
	public PlatformAccountServiceImpl()
	{
		...
		mMapper = (PlatformAccountMapper)BeanHelper.getBean("platformAccountMapper");
	}
	...
	public static IAccountService getInstance()
	{
        if (mInstance == null)
        {
            synchronized (PlatformAccountServiceImpl.class)
            {
        		if (mInstance == null)
        			mInstance = new PlatformAccountServiceImpl();
            }
        }
		return mInstance;
	}
	...
	public Account getAccount(String acc) throws AccountException, RedisException, SqlException
	{
		...
		try
		{
			account = redisSystem.get(Account.class, String.format("account:platform:%s", acc));
			if (account != null)
				return account;
		}
		catch (Exception e)
		{
			throw new AccountException("get platform account from redis fail"); 
		}
		try
		{
			account = mMapper.getAccountByAccount(acc);
			if (account != null)
				redisSystem.setExpire(String.format("account:platform:%s", acc), account, 30 * 60);
		}
		catch (Exception e)
		{
			throw new AccountException("get platform account from sql fail"); 
		}
		return account;
	}
	...
	public Player login(String acc, String pwd, boolean auto) throws AccountException, UserException, RedisException, SqlException
	{
		...
		if (account == null)
		{
			if (auto)
			{
				if (!register(acc, pwd))
				{
					throw new AccountException("register platform account fail");
				}
			...
			{
				throw new AccountException("register platform account is not exist");
			}
		}
		String token = account.getToken();
		if (!pwd.equals(token))
		{
			throw new AccountException("platform password is not correct");
		}
		Redis redisSystem = App.getSubSystem(Redis.class, SystemId.SYSTEM_REDIS);
		if (redisSystem == null)
		{
	...
	public boolean register(String acc, String pwd)
	{
		...
		try
		{
			...
			if (acc != null)
			{
				throw new AccountException("platform account already exist");
			}
			account = Account.valueOf(acc, pwd);
			success = mMapper.createAccount(account);
			if (!success)
			{
				throw new AccountException("platform account create fail");
			}
